...
function TechTile({ tech }: { tech: Tech }) {
  ...
  return (
    <div className="group relative">
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-[calc(100%+10px)] left-1/2 z-10 -translate-x-1/2 translate-y-1.5 whitespace-nowrap rounded-[11px] bg-ink px-3 py-2 text-center opacity-0 shadow-[0_12px_28px_-10px_rgba(20,18,50,0.5)] transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100"
      >
...
function TechGrid() {
  return (
    <div className="flex flex-col gap-7">
      {stack.map((group) => (
        <div key={group.label}>
          <p className="mb-3.5 font-mono text-xs tracking-[1.5px] text-muted-ink">
            {group.label}
          </p>
